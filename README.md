...
# Encryption
<p align="center">
  <img src="https://github.com/ShallWeMove/Overflow2024/assets/52898220/6e52c476-107e-485f-9932-5afb35486a32" width="800"/>
</p>

**ShallWeMove's smart contract encrypts and securely delivers cards 100% on-chain, with no backend.**
shallwemove's encryption goes through the following process
1. The client generates its own asymmetric key.
2. The client sends the public key of the asymmetric key generated by the client.
3. The smart contract draws a random card from the card deck using Sui Randomness.
4. The smart contract encrypts the card with the client's public key.
5. the Client receives the encrypted card
6. decrypts the encrypted card using the client's secret key.
